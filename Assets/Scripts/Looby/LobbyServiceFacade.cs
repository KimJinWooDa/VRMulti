...
{
    /// <summary>
    /// 이것은 로비 API로의 직접적인 호출과 실제로 원하는 결과 간의 추상화 계층입니다.
    /// 즉, 로비 API와 상호작용하는 코드가 있을 때 해당 코드를 더 쉽고 간편하게 만들기 위한 계층을 제공
    /// 이를 통해 코드를 유지 보수하고 업데이트하는 것이 더 간편해지고,
    /// API 변경 사항이 있을 때도 해당 변경에 대응하기 쉬워집니다.
    /// 이를 통해 코드 유지 관리를 개선하고 개발 생산성을 향상시킬 수 있습니다.
    /// </summary>
    public class LobbyServiceFacade : IDisposable, IStartable
    {
        ...
        /// <summary>
        /// Attempt to update lock state of a given lobby.
        /// </summary>
        public async Task UpdateLobbyLockAsync(bool isLocked)
        {
            if (!m_RateLimitQuery.CanCall)
            {
                return;
            ...
            try
            {
                var result = await m_LobbyApiInterface.UpdateLobby(CurrentUnityLobby.Id, dataCurr, isLocked);

                if (result != null)
                {
                    CurrentUnityLobby = result;
                    Debug.Log(isLocked ? "Locked lobby" : "Unlocked lobby" );
                }
            }
            catch (LobbyServiceException e)
            {
