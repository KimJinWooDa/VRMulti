...
namespace disguys.ApplicationLifecycle
{
   /// <summary>
    /// 애플리케이션의 진입점으로, 공통 의존성을 루트 DI(scope)에 바인딩하는 역할을 합니다.
    /// DI는 의존성 주입(dependency injection)의 약자로,
    /// 객체 간의 의존성을 줄이고 유연성을 높여주는 디자인 패턴입니다.
    /// 이 함수는 애플리케이션의 실행 전에 실행되며,
    /// 이후에 생성되는 객체들은 이 DI(scope)를 참조하여 필요한 의존성을 주입받게 됩니다.
